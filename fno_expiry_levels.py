...
st.caption("Reference levels are the previous monthly expiry day's high and low. "
           "The Signal column is a mechanical score from EMA / RSI / MACD / DI and the level "
           "status — it is information, not a recommendation, and it knows nothing about your "
           "position size or risk.")
...
def style_table(df):
    ...
    view["SECTOR"] = view["SECTOR"].replace({"": "—"}).fillna("—")
    # once a stock has broken out the range position is unbounded and unhelpful — cap the display
    view["RANGE_POS"] = view["RANGE_POS"].clip(-100, 200)
    sty = view.style
